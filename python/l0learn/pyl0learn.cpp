...
py_fitmodel L0LearnFit_sparse_wrapper(
    const sparse_mat &X, const arma::vec &y, const std::string &Loss,
  ...
  return py_fitmodel(L0LearnFit<arma::sp_mat>(
      X.to_arma_object(), y, Loss, Penalty, Algorithm, NnzStopNum, G_ncols,
...
PYBIND11_MODULE(l0learn, m) {
  ...
  py::class_<sparse_mat>(m, "_sparse_mat")
      .def(py::init<const arma::uvec &, const arma::uvec &, const arma::vec &,
