...
template <class T>
CDL012SquaredHingeSwaps<T>::CDL012SquaredHingeSwaps(const T& Xi, constEigen::VectorXd& yi, const Params<T>& Pi) : CDSwaps<T>(Xi, yi, Pi) {
...
FitResult<T> CDL012SquaredHingeSwaps<T>::Fit() {
   ...
   Eigen::VectorXd onemyxb = result.onemyxb;
    ...
    for (std::size_t t = 0; t < this->MaxNumSwaps; ++t) {
        ...
        for (auto& j : NnzIndices) {
           ...
           Eigen::VectorXd onemyxbnoj = onemyxb + this->B.coeffRef(j) * *(this->y) % matrix_column_get(*(this->X), j);
            ...
            for(std::size_t i = 0; i < this->p; ++i) {
                if(this->B.coeffRef(i) == 0 && i>=this->NoSelectK) {
                    ...
                    if (std::abs(partial_i) >= this->lambda1 + stl0Lc){
                       ...
                       Eigen::VectorXd onemyxbnoji = onemyxbnoj;
